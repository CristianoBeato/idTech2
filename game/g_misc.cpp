...
static void func_clock_format_countdown (edict_t *self)
{
	...
	if (self->style == 1)
	{
		...
		if (self->message[3] == ' ')
			const_cast<char*>(self->message)[3] = '0';
		return;
	}
	...
	if (self->style == 2)
	{
		...
		if (self->message[3] == ' ')
			const_cast<char*>(self->message)[3] = '0';
		if (self->message[6] == ' ')
			const_cast<char*>(self->message)[6] = '0';
		return;
	}
}
...
void func_clock_think (edict_t *self)
{
		...
		((self->spawnflags & 2) && (self->health < self->wait)))
	{
		...
		{
			const char *savetarget = nullptr;
			const char *savemessage = nullptr;
...
void SP_func_clock (edict_t *self)
{
	...
	self->message = static_cast<const char*>( gi.TagMalloc (CLOCK_MESSAGE_SIZE, TAG_LEVEL) );
